...
export default function Location({ t = ar }: { t?: Messages }) {
  const l = t.location;
  return (
    <section id="contact" className="px-5 py-24 lg:px-8 lg:py-36">
      <div className="mx-auto flex max-w-[1200px] flex-col gap-[18px] lg:grid lg:grid-cols-[1fr_1.3fr] lg:items-stretch lg:gap-12">
        <Reveal className="flex flex-col gap-[18px] lg:gap-6">
          ...
          <Image
            ...
            height={1024}
            sizes="500px"
            className="hidden h-[220px] w-full rounded-[1.5rem] object-cover lg:block"
          />
          <div className="bezel [--r:1.5rem] lg:hidden"><Map l={l} className="bezel-core h-[220px]" /></div>
          <div className="bezel [--r:1.5rem]"><div className="bezel-core">
            {l.hours.map((row) => (
              <div
                key={row.day}
                className="flex justify-between border-b border-line/70 px-4 py-3.5 text-sm last:border-b-0 lg:px-5 lg:py-4 lg:text-base"
              >
                <span>{row.day}</span>
        ...
        <Reveal delay={0.1} className="hidden lg:block">
          <div className="bezel h-full [--pad:0.5rem] [--r:2.25rem]"><Map l={l} className="bezel-core h-full min-h-[560px]" /></div>
        </Reveal>
      </div>
    </section>
  );
